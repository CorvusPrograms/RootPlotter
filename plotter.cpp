...
struct ProcessMaker{
  ...
  template<typename T>
  void addProcessor(const std::string& name){processors[name] = std::unique_ptr<T>(new T());}
   Processor* getProcessor(const std::string& s){
        return processors[s];
        }
  const Processor* operator[](const std::string& s) const {return processors[s];}
}
...
struct SimpleOverlayPlotter : public Processor {
  ...
  void createVisual(Histogram &h, const DataOptions &opts) override {
    ...
    for (const auto &source : sources) {
      if (!source->file->GetListOfKeys()->Contains(hinstance.name.c_str())) {
        throw std::runtime_error(
                                 fmt::format("Could not find histogram {} in data source {} with path {}", hinfo.name, source->name, source->path));
      }
    ...
    if (hinfo.log_x) {h.canvas->SetLogx();}
    if (hinfo.log_y) {h.canvas->SetLogy();}
...
int main(int argc, char *argv[]) {
  ...
  fmt::print("Opening file {}\n", inpath);
    ...
    if (file.keys.empty()) {
      throw std::runtime_error(fmt::format("File {} does not contain any keys", inpath));
